    ...
    <div class="card">
                ...
                <thead>
                    <tr class="font-medium-2" style="cursor:pointer">
                        ...
                        <th wire:click.prevent="sortBy('name')" class="font-medium-1">
                            ...
                            NOMBRE COMPLETO @include('partials._sort-icon',['field'=>'name'])
                ...
                <tbody>
                    @foreach ($infecteds as $infected)
                        <tr class="font-medium-2">
                            <td class="text-center align-middle"><b>{{ $infected->id }}</b> </td>

                            <td class="align-middle">{{ $infected->user->full_name }}</td>
                            <td class="align-middle text-bold">{{ $infected->user->dni }}</td>
                            <td class="text-center ">
                                @if ($infected->status == 'ACTIVO')
                                    <div class="badge badge-success text-bold-600">{{ $infected->status }}</div>

                                @elseif($infected->status=='INACTIVO')
                            ...
                            <td class="text-bold-600 text-center">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    @can('show', $infected)
                                        <a href="{{ route('infecteds.show', $infected->id) }}"
                                    ...
                                    @endcan
                                    @can('update', $infected)
                                        <a href="{{ route('infecteds.edit', $infected->id) }}"
                                            class="btn btn-icon btn-success" data-toggle="tooltip" data-placement="top"
                                            data-original-title="Editar">
                                            <i class="bx bx-edit"></i>
                                        </a>
                                    @endcan
                                    @can('forceDelete', $infected)
                                        <a style="cursor:pointer" data-target="#modal-delete-{{ $infected->id }}"
                            ...
                            <div>@include('infecteds.modal')</div>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        @if ($infecteds->count())
            <div class="card-footer">
                {{-- Viewing {{ $infecteds->firstItem() }} - {{ $infecteds->lastItem() }} of {{ $infecteds->total() }} entries --}}
                {{ $infecteds->links('partials.custom-pagination') }}
            </div>
