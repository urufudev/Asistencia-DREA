          ...
          <table  class="table table-hover mb-1 ">
            ...
            <tbody>
              ...
              <tr class="font-medium-2"> 
                <td class="text-center align-middle"><b>{{ $position->id }}</b> </td>
                <td class="align-middle">{{ $position->name }}</td>
                <td class="align-middle">{{ $position->description }}</td>
                ...
                <td class="text-bold-600 text-center">
                    <div class="btn-group" role="group" aria-label="Basic example">
                      @can('update', $position)    
                      <a href="{{route('positions.edit',$position->id)}}" class="btn btn-icon btn-success" data-toggle="tooltip" data-placement="top" data-original-title="Editar">
                          <i class="bx bx-edit"></i>
                        </a>
                       @endcan 
                        @can('forceDelete', $position)  
                        <a style="cursor:pointer" data-target="#modal-delete-{{$position->id}}" data-toggle="modal" class="btn btn-icon btn-danger" data-placement="top" data-toggle="tooltip" title="Eliminar" >
                ...
                <div >@include('positions.modal')</div>
              </tr>
              @endforeach
            </tbody>
          </table>
...
{{--                 Viewing {{ $positions->firstItem() }} - {{ $positions->lastItem() }} of {{ $positions->total() }} entries
 --}}
                {{ $positions->links('partials.custom-pagination') }}
            </div>
