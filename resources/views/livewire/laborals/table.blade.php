          ...
          <table  class="table table-hover mb-1 ">
            ...
            <tbody>
              ...
              <tr class="font-medium-2"> 
                <td class="text-center align-middle"><b>{{ $laboral->id }}</b> </td>
                <td class="align-middle">{{ $laboral->name }}</td>
                <td class="align-middle">{{ $laboral->description }}</td>
                ...
                <td class="text-bold-600 text-center">
                    <div class="btn-group" role="group" aria-label="Basic example">
                      @can('show', $laboral)    
                      <a href="{{route('laborals.show',$laboral->id)}}" class="btn btn-icon btn-info" data-toggle="tooltip" data-placement="top" data-original-title="Ver">
                          <i class="bx bx-show"></i>
                        </a>
                       @endcan
                      @can('update', $laboral)    
                      <a href="{{route('laborals.edit',$laboral->id)}}" class="btn btn-icon btn-success" data-toggle="tooltip" data-placement="top" data-original-title="Editar">
                          <i class="bx bx-edit"></i>
                        </a>
                       @endcan 
                        @can('forceDelete', $laboral)  
                        <a style="cursor:pointer" data-target="#modal-delete-{{$laboral->id}}" data-toggle="modal" class="btn btn-icon btn-danger" data-placement="top" data-toggle="tooltip" title="Eliminar" >
                ...
                <div >@include('laborals.modal')</div>
              </tr>
              @endforeach
            </tbody>
          </table>
...
{{--                 Viewing {{ $laborals->firstItem() }} - {{ $laborals->lastItem() }} of {{ $laborals->total() }} entries
 --}}
