...
<div class="modal fade text-left" id="modal-delete_user-{{$item->id}}" tabindex="-1" role="dialog"
                    aria-labelledby="myModalLabel120" aria-hidden="true">
                    {{-- {{Form::open(['route' => ['events.destroy', $item->id],'method'=>'delete'])}} --}}

                    {!! Form::open(['route'=> ['events.destroyuser', $item->id], 'method'=>'DELETE']) !!}


                    {{-- {{Form::Open(array('action' =>array('EventController','destroy',$item->id),'method'=>'delete'))}} --}}
                    {{-- {{Form::Open(['route'=>'events.delete',$item->id]),'method'=>'delete'}} --}}
